...
def rejudge_trace(
    ...
    """Re-evaluate a stored trace and refresh its evaluation results."""
    ...
    trace_dict = {
        ...
        "model": trace.get("model_name") or "unknown",
        "provider": metadata.get("provider", "litellm"),
        "input_messages": [{"role": "user", "content": prompt}] if prompt else [],
        "output_text": trace.get("response"),
        "latency_ms": trace.get("latency_ms", 0.0),
        "token_usage": {"total_tokens": trace.get("tokens_used")}
        if trace.get("tokens_used") is not None
        else None,
        "metadata": metadata,
    }
